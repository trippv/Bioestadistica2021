...
# Añadir estadisticos y valores de significancia a gráficos con `ggpubr`
...
Sin embargo, la función que mas nos interesa de ggpubr es la función `stat_compare_means()`la cual puede ser aplicada en un objetivo ggpubr o ggplot:
...
Alternativamente, se puede generar los contrastes de la siguiente manera:

```{r}
contrastes <- list( c("Placebo", "Dosis baja"), c("Placebo", "Dosis alta"), c("Dosis baja", "Dosis alta") )

ggplot(Viagratab, aes(x = dosis, y = libido, color = dosis)) + 
  geom_boxplot(width = 0.2)+
  geom_jitter(width = 0.1)+
  stat_compare_means(method = "anova", label.x = 0.5, label.y = 9)+
   stat_compare_means(label = "p.signif", method = "t.test",comparisons = contrastes) +
  theme_classic()+
  theme(legend.position = "none")
```
